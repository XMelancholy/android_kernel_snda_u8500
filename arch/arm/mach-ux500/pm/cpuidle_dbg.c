...
void ux500_ci_dbg_detect_wake_late(ktime_t t_est_wake, int target)
{
	s64 diff_us;
	...
	diff_us = ktime_us_delta(ktime_get(), t_est_wake);

	if (diff_us > 1000)
		pr_warning("dbx500-cpuidle: CPU%d wakes %lld us too "
			   "late from sleep state: %d\n",
			   smp_processor_id(), diff_us, target);
}
...
void ux500_ci_dbg_log_post_mortem(int ctarget, int prepare_target,
				  ...
				  ktime_t est_wake, int sleep, bool is_last)
{
	...
	cpuidle_post_mortem[this_cpu].status[idx].prepare_state =
		prepare_target;
...
static void __init setup_debugfs(void)
{
	...
	if (IS_ERR_OR_NULL(debugfs_create_bool("detect_wake_late",
					       S_IWUGO | S_IRUGO, cpuidle_dir,
					       &detect_wake_late)))
		goto fail;
...
void __init ux500_ci_dbg_init(struct device *dev,
			      struct cstate *cpuidle_cstates,
			      int cpuidle_cstates_len)
{
	...
	cstates_len = cpuidle_cstates_len;

	if (deepest_allowed_state > cstates_len)
		deepest_allowed_state = cstates_len;

	for_each_possible_cpu(cpu) {
		sh = devm_kzalloc(dev, sizeof(struct state_history),
						      GFP_KERNEL);
		sh->states = devm_kzalloc(dev,
					  sizeof(struct state_history_state)
					  * cstates_len,
					  GFP_KERNEL);
	...
	uart_base = devm_ioremap(dev, baseaddr, SZ_4K);
	BUG_ON(!uart_base);
