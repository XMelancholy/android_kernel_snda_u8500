...
static int cyttsp_ts_resume(struct cyttsp *ts)
{
	int retval = 0;
	struct cyttsp_xydata xydata;

	if (ts->pdata->use_sleep && (ts->state !=
		CY_ACTIVE_STATE)) {
		if (ts->pdata->wakeup) {
			retval = ts->pdata->wakeup();
			if (retval < 0)
				dev_err(ts->dev, "%s: Error, wakeup failed!\n",
					__func__);
		} else {
			dev_err(ts->dev,
					"%s: Error, wakeup not implemented check board file\n",
					__func__);
			retval = -ENOSYS;
		}
		if (retval >= 0) {
...
static int cyttsp_ts_suspend(struct cyttsp *ts)
{
	int retval = 0;

	if (ts->pdata->use_sleep &&
					(ts->state == CY_ACTIVE_STATE)) {
		retval = ttsp_send_command(ts, CY_DEEP_SLEEP_MODE);
		if (!(retval < 0))
			ts->state = CY_SLEEP_STATE;
...
static void cyttsp_ts_late_resume(struct early_suspend *h)
{
	...
	mutex_lock(&ts->input->mutex);
	if (ts->input->users && ts->suspended) {
		ts->suspended = false;
		enable_irq(ts->irq);
		if (cyttsp_ts_resume(ts) < 0)
			dev_err(ts->dev, "%s: Error, cyttsp_resume.\n",
				__func__);
	}
	mutex_unlock(&ts->input->mutex);
}
...
static int cyttsp_reset_controller(struct cyttsp *ts)
{
	...
	ret = gpio_request(ts->pdata->rst_gpio, "reset_pin");
	if (ret)
		return ret;

	ret = gpio_direction_output(ts->pdata->rst_gpio, 1);
	if (ret < 0)
		goto out;
			    ...
			    struct device *dev, int irq, size_t xfer_buf_size)
{
	...
	ts->regulator = regulator_get(ts->dev, "vcpin");
	if (IS_ERR(ts->regulator)) {
		dev_err(ts->dev, "%s: Error, regulator_get failed\n", __func__);
		goto error_regulator_get;
	}
	error = regulator_enable(ts->regulator);
	if (error < 0) {
		dev_err(ts->dev, "%s: regulator enable failed\n", __func__);
	...
	error = cyttsp_reset_controller(ts);
	if (error)
			goto err_free_irq;
