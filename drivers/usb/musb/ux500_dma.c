				...
				dma_addr_t dma_addr, u32 len)
{
	...
	/* In RX case for mode1, program the transfer length to be a multiple
	 * of packet size. To read the remaining bytes we need to reprogram in
	 * PIO/mode0.
	 */
	if (ux500_channel->is_tx == 0 && mode == 1)
		len = (len / packet_sz) * packet_sz;
	...
	ux500_channel->channel.prog_len = ux500_channel->cur_len;
