		...
		u32 len)
{
	...
	/*
	 * Do write barrier before updating index to ensure
	 * that the index is not updated before modem can
	 * read the data.
	 */
	smp_wmb();
	*rb->wi = cpu_to_le32((wi + cpylen + cpylen2 - notcpy) % rb->size);
	/*
	 * Also do write barrier after updating the index,
	 * because the modem will receive an interrupt
	 * and data must really be written to shared memory
...
static int extract_ringbuf(struct ringbuf *rb, void __user *to, u32 len)
{
	...
	if (ri <= wi) {
		...
		*rb->ri = cpu_to_le32(ri + len - notcpy);
		/*
		 * copy_to_user don't need a write barrier, but
		 * before triggering an interrupt on the modem for
		 * index change, we want to make sure that the
		 * index is really updated in memory, hence this
		 * write barrier.
		 */
		smp_wmb();
	 ...
	 size_t count, loff_t *f_pos)
{
	...
	if (STATE_IS_PENDING(dev)) {
		...
		if (STATE_IS_REMOTE_TEARDOWN(dev)) {
			xdev_dbg(dev, "received remote_shutdown "
					"indication (1)\n");
			ret = -ESHUTDOWN;
			goto read_error;
		}
