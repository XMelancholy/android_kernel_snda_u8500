...
#define TAOS_SENSOR_TSL27713_NAME				"tsl27713"
#define TAOS_DEVICE_NAME								TAOS_SENSOR_TSL27713_NAME
#define TAOS_DEVICE_ID									TAOS_SENSOR_TSL27713_NAME
#define TAOS_INPUT_NAME									TAOS_SENSOR_TSL27713_NAME
...
info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "Chip: TAOS %s\nVersion: %s\n",
			TAOS_SENSOR_TSL27713_NAME, TAOS_SENSOR_INFO);
}
...
static struct attribute_group tsl27713_attribute_group = {
		...
		const struct i2c_device_id *idp)
{
	...
	if (taos_datap == NULL) {
		printk("[tsl27713] taos_datap == NULL\n");
	}

	ret = sysfs_create_group(&taos_datap->input_dev->dev.kobj,
			&tsl27713_attribute_group);
	if (!ret)
		goto exit;
...
static int taos_get_lux(void)
{
	...
	raw_clear *= (taos_cfgp->scale_factor)*5;
	raw_ir *= (taos_cfgp->scale_factor)*2;
