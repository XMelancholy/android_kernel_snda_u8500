...
static const struct nla_policy nl80211_policy[NL80211_ATTR_MAX+1] = {
	...
	[NL80211_ATTR_SCAN_RSSI_FILTER] = { .type = NLA_U32 },
};
...
static int nl80211_trigger_scan(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (info->attrs[NL80211_ATTR_SCAN_RSSI_FILTER])
		request->rssi_treshold = nla_get_u32(
				info->attrs[NL80211_ATTR_SCAN_RSSI_FILTER]);
...
static int nl80211_set_data_filter(struct sk_buff *skb, struct genl_info *info)
{
	struct cfg80211_registered_device *rdev = info->user_ptr[0];
	int err;

	if (!info->attrs[NL80211_ATTR_DATA_FILTER])
		return -EINVAL;

	err = -EOPNOTSUPP;
	if (rdev->ops->set_data_filter) {
		err = rdev->ops->set_data_filter(&rdev->wiphy,
				nla_data(info->attrs[NL80211_ATTR_DATA_FILTER]),
				nla_len(info->attrs[NL80211_ATTR_DATA_FILTER]));
	}
...
static struct genl_ops nl80211_ops[] = {
	...
	{
		.cmd = NL80211_CMD_SET_DATA_FILTER,
		.doit = nl80211_set_data_filter,
		.policy = nl80211_policy,
		.flags = GENL_ADMIN_PERM,
		.internal_flags = NL80211_FLAG_NEED_NETDEV |
				  NL80211_FLAG_NEED_RTNL,
