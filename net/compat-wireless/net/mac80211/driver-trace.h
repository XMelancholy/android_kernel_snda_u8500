...
TRACE_EVENT(drv_set_data_filter,
	TP_PROTO(struct ieee80211_local *local, void *data,
		 int len),

	TP_ARGS(local, data, len),
	TP_STRUCT__entry(
		LOCAL_ENTRY
		__filed(int, len)
	),
	...
	TP_printk(
		LOCAL_PR_FMT "data packet filter len:%d:",
		LOCAL_PR_ARG, __entry->len
	)
);
