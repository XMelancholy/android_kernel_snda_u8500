...
/*  U8500, A9500 and U9540 common io_desc */
...
/* U8500 and A9500 IO map specific description */
...
void u8500_reset_ip(unsigned char per, unsigned int ip_mask)
{
	...
	if (cpu_is_ux540_family()) {
		prcc_rst_set = __io_address(per_clkrst_base[per] +
				U9540_PRCC_K_SOFTRST_SET);
		prcc_rst_clr = __io_address(per_clkrst_base[per] +
				U9540_PRCC_K_SOFTRST_CLR);
	} else {
		prcc_rst_set = __io_address(per_clkrst_base[per] +
				UX500_PRCC_K_SOFTRST_SET);
		prcc_rst_clr = __io_address(per_clkrst_base[per] +
				UX500_PRCC_K_SOFTRST_CLR);
	}
