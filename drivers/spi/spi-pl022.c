...
pl022_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	pl022->vcore = regulator_get(&adev->dev, "vcore");
	if (IS_ERR_OR_NULL(pl022->vcore)) {
		dev_warn(&adev->dev, "regulator vcore not found");
		pl022->vcore = NULL;
	} else {
		if (regulator_enable(pl022->vcore)) {
			regulator_put(pl022->vcore);
			pl022->vcore = NULL;
		}
	}
