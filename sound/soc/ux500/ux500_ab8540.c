...
static int old_master_clock_sel;
...
static const char * const enum_dis_ena[] = {
	"Disabled",
	"Enabled"
};
...
static int ux500_ab8540_power_control_inc(void)
{
	int ret = 0;
	...
	ab8540_power_count++;
	pr_debug("%s: ab8540_power_count changed from %d to %d",
		__func__,
		ab8540_power_count-1,
		ab8540_power_count);

	if (ab8540_power_count == 1) {
		...
		ret = clk_set_parent(clk_ptr_intclk,
				(master_clock_sel == 0) ? clk_ptr_sysclk :
						clk_ptr_ulpclk);
		if (ret != 0) {
		...
		if (ret != 0) {
			pr_err("%s: ERROR: clk_enable failed (ret = %d)!",
					__func__, ret);
			ret = -EIO;
			ab8540_power_count = 0;
			goto clk_err;
		}
		old_master_clock_sel = master_clock_sel;


		/* Power on audio-parts of AB8540 */
		ret = ab8540_audio_power_control(true);
	}



	/*check clok modif*/
	if (master_clock_sel != old_master_clock_sel) {

		pr_debug("%s: audio master_clock switch", __func__);


		/* disable audio-clock */
		ret = clk_set_parent(clk_ptr_intclk,
				(old_master_clock_sel == 0) ? clk_ptr_sysclk :
						clk_ptr_ulpclk);
		if (ret != 0) {
			pr_err("%s: ERROR: Setting master-clock to %s failed (ret = %d)!",
				__func__,
				(old_master_clock_sel == 0) ?
				"SYSCLK" : "ULPCLK",
				ret);
			ret = -EIO;
			goto clk_err;
		}
		pr_debug("%s: disabling old master-clock (%s).",
			__func__,
			(old_master_clock_sel == 0) ? "SYSCLK" : "ULPCLK");
		...
		pr_err("%s: Enabling master-clock (%s).",
			__func__,
			(master_clock_sel == 0) ? "SYSCLK" : "ULPCLK");
		ret = clk_enable(clk_ptr_audioclk);
		if (ret != 0) {
...
static void ux500_ab8540_power_control_dec(void)
{
	pr_debug("%s: Enter.\n", __func__);
	...
	if (ab8540_power_count == 0) {
		/* Power off audio-parts of AB8540 */
		ab8540_audio_power_control(false);
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((clk_ptr_sysclk12buf1 == NULL) || (clk_ptr_msp4clk == NULL)) {
		pr_err("%s: ERROR: clock not available (%p/%p)",
			__func__, clk_ptr_sysclk12buf1, clk_ptr_msp4clk);
		return -ENODEV;
	}
	...
	if (val == 1) {
		if (clk_enable(clk_ptr_sysclk)) {
			pr_err("%s: ERROR:  clk_enable(ab8540-sysclk) FAILED\n",
					__func__);
			return -EIO;
		}
...
static const struct snd_kcontrol_new msp4_clk_control = \
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = ux500_ab8540_power_control_inc();
	if (ret < 0) {
	...
	ab8540_audio_anc_configure(codec, apply_fir, apply_iir);
	...
	mutex_unlock(&codec->mutex);
	ux500_ab8540_power_control_dec();
...
static int amic3_reg_control_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	return amic_reg_control_get(ucontrol, AMIC_3);
}

static int amic3_reg_control_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	return amic_reg_control_put(ucontrol, AMIC_3);
}
...
static const struct snd_kcontrol_new mic3_regulator_control = \
	SOC_ENUM_EXT("Mic 3 Regulator", soc_enum_amicconf,
		amic3_reg_control_get, amic3_reg_control_put);
			...
			struct snd_kcontrol *k, int event)
{
	if (SND_SOC_DAPM_EVENT_ON(event))
		ux500_ab8540_power_control_inc();
	else
		ux500_ab8540_power_control_dec();
...
static int dapm_amic3reg_event(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *k, int event)
{
	return dapm_amicreg_event(AMIC_3, event);
}
...
static const struct snd_soc_dapm_widget ux500_ab8540_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SUPPLY("AMIC2 Regulator",
			...
			SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_SUPPLY("AMIC3 Regulator",
			SND_SOC_NOPM, 0, 0, dapm_amic3reg_event,
			SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
...
static const struct snd_soc_dapm_route ux500_ab8540_dapm_intercon[] = {

	/* Power AB8540 audio-block when AD/DA is active */
	{"DAC", NULL, "AUDIO Regulator"},
	{"ADC", NULL, "AUDIO Regulator"},

	/* Power AB8540 audio-block when voice specific AD/DA is active */
	{"DAC voice", NULL, "AUDIO Regulator"},
	{"ADC voice", NULL, "AUDIO Regulator"},

	/* Power AB8540 audio-block when LineIn is active */
	...
	{"MIC3 Input", NULL, "AMIC3 Regulator"},
...
static int add_widgets(struct snd_soc_codec *codec)
{
	...
	ret = snd_soc_dapm_new_controls(&codec->dapm,
			ux500_ab8540_dapm_widgets,
			ARRAY_SIZE(ux500_ab8540_dapm_widgets));
	if (ret < 0) {
	...
	ret = snd_soc_dapm_add_routes(&codec->dapm,
				ux500_ab8540_dapm_intercon,
				ARRAY_SIZE(ux500_ab8540_dapm_intercon));
	if (ret < 0) {
			...
			struct snd_pcm_hw_params *params)
{
	...
	ab8540_audio_set_bit_delay(codec_dai->codec, AB8540_IF1_DAI_ID, 1);
	ux500_msp_dai_set_data_delay(cpu_dai, MSP_DELAY_1);

	if (driver_mode == DRIVERMODE_NORMAL) {
		codec_slot_width = cpu_slot_width;
		ab8540_audio_set_word_length(codec_dai->codec,
				AB8540_IF1_DAI_ID, codec_slot_width);
	...
	ret = snd_soc_dai_set_tdm_slot(cpu_dai, tx_slots, rx_slots,
			slots, cpu_slot_width);
	if (ret)
		return ret;

	pr_debug("%s: CODEC-DAI TDM: TX=0x%04X RX=0x%04x\n",
		__func__, tx_slots, rx_slots);
	ret = snd_soc_dai_set_tdm_slot(codec_dai, tx_slots, rx_slots,
			slots, codec_slot_width);
	if (ret)
		return ret;

	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		pr_debug("%s: Setup IF2 for FM-radio.\n", __func__);
		fmt_if2 = SND_SOC_DAIFMT_CBM_CFM | SND_SOC_DAIFMT_I2S;
		ret = ab8540_audio_setup_if2(codec_dai, fmt_if2, 16, 1);
		if (ret)
			return ret;
	}
...
struct snd_soc_ops ux500_ab8540_ops[] = {
	{
	.hw_params = ux500_ab8540_hw_params,
	.startup = ux500_ab8540_startup,
	.shutdown = ux500_ab8540_shutdown,
	}
};

/* ux500_ab8540_voice_hw_params
 * This function is used to configure the voice interface of ab8540
 */
int ux500_ab8540_voice_hw_params(struct snd_pcm_substream *substream,
			struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
	unsigned int fmt;
	int channels, rate, ret = 0, slots, slot_width;
	...
	pr_debug("%s:  channels = %d rate = %d\n", __func__, channels, rate);

	/* rate = 16000; //Fixed sample rate at 16kHz */
	ret = snd_soc_dai_set_sysclk(codec_dai, 0, rate, 0);
	if (ret < 0) {
		pr_err("%s: ERROR: snd_soc_dai_set_sysclk failed (ret = %d)!\n",
			__func__,
			ret);
		return ret;
	}

	ab8540_audio_set_bit_delay(codec_dai->codec, AB8540_IF3_DAI_ID, 1);

	ab8540_audio_set_word_length(codec_dai->codec, AB8540_IF3_DAI_ID, 16);

	fmt = SND_SOC_DAIFMT_DSP_A   | /* TDM mode */
	      SND_SOC_DAIFMT_CBS_CFS | /* BitClock Slave and FSync Slave */
	      SND_SOC_DAIFMT_NB_NF   | /* BClk & FSync sampled on rising edge */
	      SND_SOC_DAIFMT_CONT;

	ret = snd_soc_dai_set_fmt(codec_dai, fmt);
	if (ret < 0) {
		pr_err("%s: ERROR: snd_soc_dai_set_fmt failed (ret = %d)!\n",
			__func__,
			ret);
		return ret;
	}
	...
	slots = 4;
	slot_width = 16;
	ret = snd_soc_dai_set_tdm_slot(codec_dai, 0, 0, slots, slot_width);
	if (ret)
		return ret;

	return 0;
}
struct snd_soc_ops ux500_ab8540_voice_ops[] = {
	{
	.hw_params = ux500_ab8540_voice_hw_params,
	}
};

int ux500_ab8540_machine_codec_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	snd_ctl_add(codec->card->snd_card, snd_ctl_new1(
		&mic3_regulator_control, codec));

	snd_ctl_add(codec->card->snd_card, snd_ctl_new1(
		&msp4_clk_control, codec));
	...
	clk_ptr_sysclk = clk_get(codec->dev, "ab8540-sysclk");
	if (IS_ERR(clk_ptr_sysclk)) {
		pr_err("ERROR: clk_get failed (ret = %d)!",
				-EFAULT);
		return -EFAULT;
	}
	...
	clk_ptr_sysclk12buf1 = clk_get(codec->dev, "ab9540-sysclk12Buf1");
	...
	if (IS_ERR(clk_ptr_msp4clk)) {
		pr_err("ERROR: clk_get_sys(msp4) failed (ret = %d)!", -EFAULT);
		return -EFAULT;
	}
...
void ux500_ab8540_audio_pwm_vibra(unsigned char speed_pos,
			unsigned char speed_neg)
{
	bool vibra_on_new;

	vibra_on_new = speed_pos | speed_neg;
	...
	ab8540_audio_pwm_vibra(speed_pos,
			speed_neg);
}

int ux500_ab8540_audio_gpadc_measure(struct ab8500_gpadc *gpadc,
		u8 channel, bool mode, int *value)
{
	int ret = 0;
	int adcm = (mode) ?
		AB8540_AUDIO_ADCM_FORCE_UP :
		AB8540_AUDIO_ADCM_FORCE_DOWN;
	...
	ret = ab8540_audio_set_adcm(adcm);
	if (ret < 0) {
	...
	ret |= ab8540_audio_set_adcm(AB8540_AUDIO_ADCM_NORMAL);
...
void ux500_ab8540_jack_report(int value)
{
	if (jack.jack)
		snd_soc_jack_report(&jack, value, 0xFF);
}
EXPORT_SYMBOL_GPL(ux500_ab8540_jack_report);
