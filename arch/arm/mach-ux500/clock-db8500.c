...
static DEFINE_MUTEX(sdmmchclk_mutex);
...
/* ab9540_ Touch screen/secondary camera clock operations. */
static int ab9540_clkout1_enable(struct clk *clk)
{
	int r;
	...
	if (r)
		goto regulator_failed;
	r = prcmu_config_clkout(1, PRCMU_CLKSRC_CLK38M, 4);
	if (r)
		goto config_failed;
	r = nmk_config_pin(GPIO228_CLKOUT2, false);
	if (r)
...
gpio_failed:
	(void)prcmu_config_clkout(1, PRCMU_CLKSRC_CLK38M, 0);
config_failed:
	(void)regulator_disable(clk->regulator);
regulator_failed:
	return r;
...
static void ab9540_clkout1_disable(struct clk *clk)
{
	int r;
...
static struct clkops ab9540_clkout1_ops = {
	.enable = ab9540_clkout1_enable,
	.disable = ab9540_clkout1_disable,
};
...
static int db9540_clkout0yuv_enable(struct clk *clk)
{
	...
	clk_enable(&lcdclk);
	r = prcmu_config_clkout(0, PRCMU_CLKSRC_LCDCLK, 1);
	if (r)
		goto config_failed;
	r = nmk_config_pin(GPIO227_CLKOUT1, false);
	if (r)
...
static int clkout1yuv_enable(struct clk *clk)
{
	...
	clk_enable(&lcdclk);
	r = prcmu_config_clkout(1, PRCMU_CLKSRC_LCDCLK, 1);
	if (r)
		goto config_failed;
	r = nmk_config_pin(GPIO228_CLKOUT2, false);
	if (r)
...
static int db8540_sdmmcx_ape_configurate(struct clk *clk, unsigned long rate)
{
	static bool ape_max_opp;
	static unsigned long sdmmc_rate;
	static unsigned long sdmmch_rate;
	int err = 0;
	bool raise;
	...
	if (clk->cg_sel == PRCMU_SDMMCCLK)
		sdmmc_rate = rate;
	else if (clk->cg_sel == PRCMU_SDMMCHCLK)
		sdmmch_rate = rate;
	else
		return -EINVAL;
ape_opp_check:

	if ((ape_max_opp) && (sdmmc_rate <= 100000000)
		&& (sdmmch_rate <= 200000000))
		raise = false;
	else if ((!ape_max_opp) && ((sdmmc_rate > 100000000)
		|| (sdmmch_rate > 200000000)))
		raise = true;
	else
		return err;

	err = prcmu_request_ape_opp_100_voltage(raise);
	if (err)
		pr_err("clock: %s failed to request opp voltage %s.\n",
			__func__, clk->name);
	else
		ape_max_opp = raise;
...
static int clk_enable_sdmmcxclk(struct clk *clk)
{
	int err;

	err = db8540_sdmmcx_ape_configurate(clk, 0);
	if (!err)
		err = prcmu_request_clock(clk->cg_sel, true);
	return err;
}

static void clk_disable_sdmmcxclk(struct clk *clk)
{
	db8540_sdmmcx_ape_configurate(clk, 0);
	if (prcmu_request_clock(clk->cg_sel, false))
		pr_err("clock: %s failed to disable %s.\n",
			__func__, clk->name);
}

static unsigned long clk_get_rate_sdmmcxclk(struct clk *clk)
{
	return prcmu_clock_rate(clk->cg_sel);
}

static long clk_round_rate_sdmmcxclk(struct clk *clk, unsigned long rate)
{
	return prcmu_round_clock_rate(clk->cg_sel, rate);
}

static int clk_set_rate_sdmmcxclk(struct clk *clk, unsigned long rate)
{
	int err;

	if (clk->cg_sel == PRCMU_SDMMCCLK)
		BUG_ON(rate > 200000000);
	else if (clk->cg_sel == PRCMU_SDMMCHCLK)
		BUG_ON(rate > 400000000);
	else
		return -EINVAL;

	err = db8540_sdmmcx_ape_configurate(clk, rate);
	if (!err)
		err = prcmu_set_clock_rate(clk->cg_sel, rate);
	return err;
}

static struct clkops clk_ops_sdmmcxclk = {
	.enable = clk_enable_sdmmcxclk,
	.disable = clk_disable_sdmmcxclk,
	.get_rate = clk_get_rate_sdmmcxclk,
	.round_rate = clk_round_rate_sdmmcxclk,
	.set_rate = clk_set_rate_sdmmcxclk,
};
...
static struct clk sdmmchclk = {
	.name = "sdmmchclk",
	.ops = &clk_ops_sdmmcxclk,
	.cg_sel = PRCMU_SDMMCHCLK,
	.mutex = &sdmmchclk_mutex,
};
...
static DEF_PRCMU_CLK(camclk, PRCMU_CAMCLK, 24000000);
...
static DEF_PRCMU_CLK(aclk, PRCMU_ACLK, 200000000);
...
static DEF_PER3_PCLK(9, p3_pclk9);
static DEF_PER3_PCLK(10, p3_pclk10);
/* p3_pclk11 & p3_clk12 are only valid for DB8540 */
static DEF_PER3_PCLK(11, p3_pclk11);
static DEF_PER3_PCLK(12, p3_pclk12);
...
/* I2C5 */
static DEF_PER3_KCLK(8, p3_i2c5_kclk, &i2cclk);
static DEF_PER_CLK(p3_i2c5_clk, &p3_pclk9, &p3_i2c5_kclk);

/* I2C6 */
static DEF_PER3_KCLK(9, p3_i2c6_kclk, &i2cclk);
static DEF_PER_CLK(p3_i2c6_clk, &p3_pclk10, &p3_i2c6_kclk);

/* UART3 - Only valid on DB8540 */
static DEF_PER3_KCLK(10, p3_uart3_kclk, &uartclk);
static DEF_PER_CLK(p3_uart3_clk, &p3_pclk11, &p3_uart3_kclk);

/* UART4 - Only valid on DB8540 */
static DEF_PER3_KCLK(11, p3_uart4_kclk, &uartclk);
static DEF_PER_CLK(p3_uart4_clk, &p3_pclk12, &p3_uart4_kclk);
...
static struct clk_lookup common_clocks[] = {
	...
	CLK_LOOKUP(p1_i2c1_clk, "nmk-i2c.1", "apb_pclk"),
	...
	CLK_LOOKUP(p1_i2c2_clk, "nmk-i2c.2", "apb_pclk"),
	...
	CLK_LOOKUP(p1_i2c4_clk, "nmk-i2c.4", "apb_pclk"),

	/* PERIPH 2 */
	CLK_LOOKUP(p2_i2c3_clk, "nmk-i2c.3", NULL),
	CLK_LOOKUP(p2_i2c3_clk, "nmk-i2c.3", "apb_pclk"),
	...
	CLK_LOOKUP(p3_i2c0_clk, "nmk-i2c.0", "apb_pclk"),
	...
	CLK_LOOKUP(p3_uart3_clk, "uart3", NULL),
	CLK_LOOKUP(p3_uart4_clk, "uart4", NULL),
...
static struct clk_lookup u8500_clocks[] = {
	...
	CLK_LOOKUP(ab8500_sysclk, "ab850x-codec.0", "sysclk"),
	CLK_LOOKUP(ab8500_sysclk, "u8500-shrm", "sysclk"),
	...
	CLK_LOOKUP(ab_ulpclk, "ab850x-codec.0", "ulpclk"),
	CLK_LOOKUP(ab8500_intclk, "ab850x-codec.0", "intclk"),
	CLK_LOOKUP(ab8500_intclk, "ab8500-pwm.1", NULL),
	CLK_LOOKUP(ab8500_audioclk, "ab850x-codec.0", "audioclk"),
	/* PERIPH 1 */
	CLK_LOOKUP(p1_msp3_kclk, "ab850x-codec.0", "msp3-kernel"),
	CLK_LOOKUP(p1_pclk11, "ab850x-codec.0", "msp3-bus"),
	CLK_LOOKUP(p1_msp1_kclk, "ab850x-codec.0", "msp1-kernel"),
	CLK_LOOKUP(p1_pclk4, "ab850x-codec.0", "msp1-bus"),
	CLK_LOOKUP(dsi0clk, "dsilink.0", "dsihs0"),
	CLK_LOOKUP(dsi1clk, "dsilink.1", "dsihs1"),
	CLK_LOOKUP(dsi_pll, "dsilink.2", "dsihs2"),
};
...
static struct clk_lookup u9540_sysclks[] = {
	...
	CLK_LOOKUP(ab9540_sysclk, "ab850x-codec.0", "ab9540-sysclk"),
	CLK_LOOKUP(ab9540_sysclk, "ab9540-usb.0", "ab9540-sysclk"),
	CLK_LOOKUP(ab9540_sysclk12Buf1, "ab850x-codec.0",
			"ab9540-sysclk12Buf1"),
	CLK_LOOKUP(ab8500_sysclk, "ab-iddet.0", "sysclk"),
};

static struct clk_lookup u8540_sysclks[] = {

	CLK_LOOKUP(ab9540_sysclk, "ab8540-usb.0", "ab8540-sysclk"),
	CLK_LOOKUP(ab9540_sysclk, "ab8540-codec.0", "ab8540-sysclk"),
	CLK_LOOKUP(ab9540_sysclk12Buf1, "ab8540-codec.0",
			"ab9540-sysclk12Buf1"),
};
...
static struct clk_lookup u9540_clocks[] = {
	...
	CLK_LOOKUP(dsi_pll, "mcde", "dsihs2"),
	CLK_LOOKUP(ab_ulpclk, "ab850x-codec.0", "ulpclk"),
	CLK_LOOKUP(ab9540_intclk, "ab850x-codec.0", "intclk"),
	CLK_LOOKUP(ab9540_intclk, "ab8500-pwm.1", NULL),
	CLK_LOOKUP(ab9540_audioclk, "ab850x-codec.0", "audioclk"),
	/* PERIPH 1 */
	CLK_LOOKUP(p1_msp3_kclk, "ab850x-codec.0", "msp3-kernel"),
	CLK_LOOKUP(p1_pclk11, "ab850x-codec.0", "msp3-bus"),
	CLK_LOOKUP(p1_msp1_kclk, "ab850x-codec.0", "msp1-kernel"),
	...
	CLK_LOOKUP(p2_msp4_clk, "ab850x-codec.0", "msp4"),
	CLK_LOOKUP(p2_msp4_kclk, "ab850x-codec.0", "msp4-kernel"),
	CLK_LOOKUP(p2_pclk12, "ab850x-codec.0", "msp4-bus"),
};

static struct clk_lookup u8540_clocks[] = {
	CLK_LOOKUP(clk_arm, "cpufreq-ux500", NULL),
	CLK_LOOKUP(soc0_pll, NULL, "soc0_pll"),
	CLK_LOOKUP(soc1_pll, NULL, "soc1_pll"),
	CLK_LOOKUP(ddr_pll, NULL, "ddr_pll"),
	...
	CLK_LOOKUP(i2cclk, "I2C", NULL),
	CLK_LOOKUP(sdmmcclk, "sdmmc", NULL),
	CLK_LOOKUP(sdmmchclk, "sdmmchclk", NULL),
	CLK_LOOKUP(per1clk, "PERIPH1", NULL),
	CLK_LOOKUP(per2clk, "PERIPH2", NULL),
	CLK_LOOKUP(per3clk, "PERIPH3", NULL),
	CLK_LOOKUP(per5clk, "PERIPH5", NULL),
	...
	CLK_LOOKUP(dsi1lcdclk, "mcde", "dsihs1"),
	CLK_LOOKUP(dsi_pll, "mcde", "dsihs2"),
	CLK_LOOKUP(ab_ulpclk, "ab8540-codec.0", "ulpclk"),
	CLK_LOOKUP(ab9540_intclk, "ab8540-codec.0", "intclk"),
	CLK_LOOKUP(ab9540_audioclk, "ab8540-codec.0", "audioclk"),
	CLK_LOOKUP(ab9540_intclk, "ab8500-pwm.1", NULL),
	CLK_LOOKUP(camclk, "cam", NULL),

	/* PERIPH 1 */
	CLK_LOOKUP(p1_msp3_clk, "msp3", NULL),
	CLK_LOOKUP(p1_msp3_clk, "ux500-msp-i2s.3", NULL),
	CLK_LOOKUP(p1_msp3_kclk, "ab8540-codec.0", "msp3-kernel"),
	CLK_LOOKUP(p1_msp1_kclk, "ab8540-codec.0", "msp1-kernel"),
	CLK_LOOKUP(p1_uart0_clk, "uart0", NULL),
	CLK_LOOKUP(p1_uart1_clk, "uart1", NULL),
	CLK_LOOKUP(p1_i2c1_clk, "nmk-i2c.1", NULL),
	CLK_LOOKUP(p1_msp0_clk, "msp0", NULL),
	...
	CLK_LOOKUP(p1_pclk7, "spi3", NULL),
	CLK_LOOKUP(p1_i2c4_clk, "nmk-i2c.4", NULL),
	CLK_LOOKUP(p1_pclk11, "ab8540-codec.0", "msp3-bus"),
	CLK_LOOKUP(p1_pclk4, "ab8540-codec.0", "msp1-bus"),
	CLK_LOOKUP(dsi0lcdclk, "dsilink.0", "dsihs0"),
	CLK_LOOKUP(dsi1lcdclk, "dsilink.1", "dsihs1"),
	CLK_LOOKUP(dsi_pll, "dsilink.2", "dsihs2"),
	CLK_LOOKUP(p1_pclk3, "ux500-msp-i2s.0", "buf"),
	...
	CLK_LOOKUP(p2_pclk11, "gpioblock1", NULL),
	CLK_LOOKUP(p2_msp4_clk, "msp4", NULL),
	CLK_LOOKUP(p2_msp4_clk, "ab8540-codec.0", "msp4"),
	CLK_LOOKUP(p2_msp4_kclk, "ab8540-codec.0", "msp4-kernel"),
	CLK_LOOKUP(p2_pclk12, "ab8540-codec.0", "msp4-bus"),
	...
	CLK_LOOKUP(p3_uart3_clk, "uart3", NULL),
	CLK_LOOKUP(p3_uart4_clk, "uart4", NULL),
	CLK_LOOKUP(p3_i2c5_clk, "nmk-i2c.5", NULL),
	CLK_LOOKUP(p3_i2c6_clk, "nmk-i2c.6", NULL),
...
static struct clk *db8540_dbg_clks[] __initdata = {
	/* Clock sources */
	&soc0_pll,
	&soc1_pll,
	&ddr_pll,
...
static struct clk *db8540_loader_enabled_clk[] __initdata = {
	/* periph 1 */
	&p1_uart0_clk,
	&p1_uart1_clk,
	&p1_i2c1_clk,
...
static int __init init_clock_states(void)
{
	...
	if (cpu_is_u8540()) {
		for (i = 0; i < ARRAY_SIZE(db8540_loader_enabled_clk); i++)
			if (!clk_enable(db8540_loader_enabled_clk[i]))
				clk_disable(db8540_loader_enabled_clk[i]);
	} else {
...
static void __init db8540_sdmmcx_configration(void)
{
	p1_sdi0_kclk.parent = &sdmmchclk;
	p2_sdi1_kclk.parent = &sdmmchclk;
	p3_sdi2_kclk.parent = &sdmmchclk;
	p2_sdi4_kclk.parent = &sdmmchclk;

	p1_sdi0_kclk.mutex = &sdmmchclk_mutex;
	p2_sdi1_kclk.mutex = &sdmmchclk_mutex;
	p3_sdi2_kclk.mutex = &sdmmchclk_mutex;
	p2_sdi4_kclk.mutex = &sdmmchclk_mutex;

	p1_sdi0_clk.mutex = &sdmmchclk_mutex;
	p2_sdi1_clk.mutex = &sdmmchclk_mutex;
	p3_sdi2_clk.mutex = &sdmmchclk_mutex;
	p2_sdi4_clk.mutex = &sdmmchclk_mutex;

	sdmmcclk.ops = &clk_ops_sdmmcxclk;
}
...
int __init db8500_clk_init(void)
{
	...
	if (cpu_is_u9540()) {
		...
		/*
		 * add u9540's sysclk, since this part 8540 is same as 9540.
		 */
		clkdev_add_table(u9540_sysclks,
				ARRAY_SIZE(u9540_sysclks));
		clkdev_add_table(u8540_sysclks,
				ARRAY_SIZE(u8540_sysclks));
		clkdev_add_table(u8540_clocks,
				ARRAY_SIZE(u8540_clocks));
		/*
		 * 8540 will not use the common_clock table.
		 */
		return 0;
	} else {
...
int __init db8500_clk_debug_init(void)
{
	if (cpu_is_u9540()) {
		...
		return dbx500_clk_debug_init(db8540_dbg_clks,
					     ARRAY_SIZE(db8540_dbg_clks));
