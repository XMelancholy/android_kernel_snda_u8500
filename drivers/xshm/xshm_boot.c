...
MODULE_PARM_DESC(xshm_c2c_bootaddr, "Address given to modem "
		"(through GENI register)");
...
static inline bool ok_checksum(struct xshm_ipctoc *ipctoc)
{
	...
	new_imghash = crc_ccitt(0xffff, (u8 *) shm_start, modem_bootimg_size);
...
static int write_to_shm(void)
{
	...
	for (i = 0; i < xshm_channels; i++) {
		...
		size = xshmdevs[i]->cfg.rx.ch_size;
		if (xshmdevs[i]->cfg.mode & XSHM_PACKET_MODE) {
			u32 buf_size;
			buf_size = size / xshmdevs[i]->cfg.rx.buffers;
			buf_size = rounddown(buf_size, align);
			size = buf_size * xshmdevs[i]->cfg.rx.buffers;
		}
	...
	for (i = 0; i < xshm_channels; i++) {
		...
		align = rounddown_pow_of_two(xshmdevs[i]->cfg.tx.alignment);
		ch->alignment =	align;
		ch->packets = xshmdevs[i]->cfg.tx.packets;
		ch->mtu = xshmdevs[i]->cfg.tx.mtu;
...
static int xshm_verify_config(struct xshm_channel *xcfg)
{
	...
	if (xshm_channels <= 0 || xshm_channels > XSHM_MAX_CHANNELS) {
...
static void create_devs(void)
{
	...
	for (i = 0; i < xshm_channels; i++) {
		...
		ch_rx = LEOFFS2PTR(shm_start, ipctoc->channel_offsets[i].rx);
		buf_rx = LEOFFS2PTR(shm_start, ch_rx->ipc);
		ch_tx = LEOFFS2PTR(shm_start, ipctoc->channel_offsets[i].tx);
			...
			struct genl_info *info, struct nla_policy const *policy)
{
	...
	if (nla_validate_nested(info->attrs[attr], XSHM_A_MAX, policy) != 0) {
			...
			int mode)
{
	...
	len = nla_strlcpy(name, info->attrs[XSHM_A_NAME], XSHM_NAMESZ);
	...
	if (cfg->tx.ch_size + cfg->rx.ch_size > xshm_size) {
