...
struct chnl_regs {
	...
	u16 (*map_g)(u8);
	u16 (*map_b)(u8);
	u16 (*oled_map_r)(u8, u8, void *);
	u16 (*oled_map_g)(u8, u8, void *);
	u16 (*oled_map_b)(u8, u8, void *);
	void *oled_palette_context;
	bool palette_enable;
	bool oled_palette_enable;
	bool oled_enable;
	bool background_yuv;
	bool bcd;
	bool roten;
...
struct mcde_chnl_state {
	...
	/* Count of VCMP interrupts */
	atomic_t vcmp_cnt;
	/* After what VCMP could registers be updated */
	int vcmp_cnt_wait;
	/*After what VCMP is previous frame output:ed and can be reused */
	int vcmp_cnt_wait_return;
	/* Timestamp of the last VCMP interrupt */
	ktime_t vcmp_timestamp;
	/* VSYNC if one of TE0, TE1 or BTA_TE */
	atomic_t vsync_cnt;
	ktime_t vsync_timestamp; /* Timestamp of the last VSYNC interrupt */
	int vsync_cnt_wait;
	atomic_t n_vsync_capture_listeners;
	atomic_t n_bta_te_oneshot_listeners;
