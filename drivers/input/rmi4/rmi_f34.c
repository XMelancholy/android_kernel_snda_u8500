...
static ssize_t rmi_fn_34_fw_version_show(struct device *dev,
				       struct device_attribute *attr,
				       char *buf);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(fw_version, RMI_RO_ATTR,
	       rmi_fn_34_fw_version_show, rmi_store_error),
...
static int rmi_f34_initialize(struct rmi_function_container *fc)
{
	...
	/* Get firmware version string and store it in the instance data */
	memset(f34->fw_version, 0, 5);
	retval = rmi_read_block(fc->rmi_dev, control_base_addr, f34->fw_version, 4);

	if (retval < 0) {
		dev_err(&fc->dev, "Couldn't read firmware version from 0x%x, "
			"error=%d.\n", control_base_addr,
			retval);
		return retval;
	}
				...
				size_t count)
{
	...
	case WRITE_CONFIG_BLOCK:
		...
		if (instance_data->cmd == ENABLE_FLASH_PROG) {
			dev_info(dev, "cmd: enable flash prog\n");
			instance_data->inflashprogmode = true;
		}
		if (instance_data->cmd == ERASE_ALL)
			dev_info(dev, "cmd: erase all\n");
