...
struct delayed_work reset_wk;
struct rmi_device_platform_data *dev_pdata; 

static int f11_reset_count = 0;
...
static ssize_t f11_ctrlRegsVal_show(struct device *dev,
					struct device_attribute *attr,
					char *buf);

static ssize_t f11_queryRegsVal_show(struct device *dev,
					struct device_attribute *attr,
					char *buf);

static ssize_t f11_resetCnt_show(struct device *dev,
				     struct device_attribute *attr,
				     char *buf);

static ssize_t f11_ctr1_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count);
static ssize_t f11_ctr2_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count);
static ssize_t f11_ctr3_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count);
static ssize_t f11_ctr0_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(ctrlRegsVal, RMI_RO_ATTR, f11_ctrlRegsVal_show, NULL),
	__ATTR(queryRegsVal, RMI_RO_ATTR, f11_queryRegsVal_show, NULL),
	__ATTR(resetCnt, RMI_RO_ATTR, f11_resetCnt_show, NULL),

	__ATTR(ctrl0, RMI_WO_ATTR, NULL, f11_ctr0_store),
	__ATTR(ctrl1, RMI_WO_ATTR, NULL, f11_ctr1_store),
	__ATTR(ctrl2, RMI_WO_ATTR, NULL, f11_ctr2_store),
	__ATTR(ctrl3, RMI_WO_ATTR, NULL, f11_ctr3_store)
					...
					u8 finger_state, u8 n_finger)
{
	...
	/*pr_alert("%s: f_state[%d]:%d - x:%d y:%d z:%d w_max:%d w_min:%d\n",
...
static int rmi_f11_virtual_button_handler(struct f11_2d_sensor *sensor)
{
	...
	struct rmi_f11_virtualbutton_map *virtualbutton_map;
    //if(sensor != NULL) {
    //    pr_alert("%s: has_gestures %d\n", __func__, sensor->sens_query.has_gestures);
    //}
	if (sensor->sens_query.has_gestures &&
		...
		for (i = 0; i < virtualbutton_map->buttons; i++) {
			if (INBOX(x, y, virtualbutton_map->map[i])) {
                ...
                //pr_alert("%s: keycode %d, x %d, y %d\n", __func__, virtualbutton_map->map[i].code, x, y);
				return 0;
			}
		}
	}
			...
			struct f11_2d_sensor_query *query, u8 query_base_addr)
{
    ...
    //pr_alert("query1-4: %d %d %d %d", query->f11_2d_query1__4[0], 
    //            query->f11_2d_query1__4[1],   
    //            query->f11_2d_query1__4[2], 
    //            query->f11_2d_query1__4[3]);
...
static void f11_set_abs_params(struct rmi_function_container *fc, int index)
{
	...
	/* workaround: to compatible old TPK touch firmwire */
	if (y_max == 1900)
		y_max = 1740;	/* lcd bottom area */

	/* Use the max Y from the virtual values, reserve for virtual key */
	if (instance_data->sensors[index].axis_align.virtual_Y_high < y_max)
		y_max = instance_data->sensors[index].axis_align.virtual_Y_high;
...
/* virtual keys definition, key_type:key_value:center_x:center_y:x_width:y_height */
static ssize_t virtual_keys_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf,
#ifdef CONFIG_BOARD_SNDA_P0
		       __stringify(EV_KEY) ":" __stringify(KEY_MENU) ":90:1000:180:80"
		       ":" __stringify(EV_KEY) ":"__stringify(KEY_HOME) ":270:1000:180:80"
		       ":" __stringify(EV_KEY) ":"__stringify(KEY_BACK) ":450:1000:180:80"
#else
		       __stringify(EV_KEY) ":" __stringify(KEY_BACK) ":90:1000:180:80"
		       ":" __stringify(EV_KEY) ":"__stringify(KEY_HOME) ":270:1000:180:80"
		       ":" __stringify(EV_KEY) ":"__stringify(KEY_MENU) ":450:1000:180:80"
#endif
		       "\n");
}
...
static struct kobj_attribute virtual_keys_attr = {
	...
	.attr = {
		 .name = "virtualkeys.synaptics_rmi4_i2c",//need same as touchscreen driver defined name
		 .mode = S_IRUGO,
		 },
...
static int is_reset_finished = 1;
static void reset_delayed_work(struct work_struct *work) 
{
    //pr_alert("%s, in\n", __func__);
    dev_pdata->gpio_reset(dev_pdata->gpio_data);
    is_reset_finished = 1;
    //pr_alert("%s, out\n", __func__);
}
...
static int rmi_f11_init(struct rmi_function_container *fc)
{
    ...
    INIT_DELAYED_WORK(&reset_wk, reset_delayed_work);
	return 0;
...
static int rmi_f11_register_devices(struct rmi_function_container *fc)
{
	...
	for (i = 0; i < (f11->dev_query.nbr_of_sensors + 1); i++) {
		...
		* phys name for input device */
        pr_alert("%s, fn %d", __func__, fc->fd.function_number);
        if(fc->fd.function_number == 17) {
		    sprintf(f11->sensors[i].input_name, "synaptics_rmi4_i2c");
        }
        else {
...
#ifdef CONFIG_RMI4_VIRTUAL_BUTTON
        //pr_alert("%s: CONFIG_RMI4_VIRTUAL_BUTTON defined, has_gestures = %d\n", __func__, f11->sensors[i].sens_query.has_gestures);
...
/* workaround: STE U8500 i2c is not stable with large block size,
   so have to split i2c block to multiple small fregment */
static int rmi_read_block_frag(struct rmi_device *d, u16 addr, u8 *buf, int len)
{
	const static int fragsize = 4;
	int error = 0;
	int lmin;
	while (len>0) {
		lmin = min(fragsize, len);
		error = rmi_read_block(d, addr, buf, lmin);
		if (error < 0)
			break;
		addr += fragsize;
		buf += fragsize;
...
int rmi_f11_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
    ...
    if(!is_reset_finished) { 
        pr_alert("%s, reset not finished yet...\n", __func__);
        return 0;
    }

	for (i = 0; i < f11->dev_query.nbr_of_sensors + 1; i++) {
		error = rmi_read_block_frag(rmi_dev,
...
static int rmi_f11_resume(struct rmi_function_container *fc)
{
    ...
    if( rmi_f11_get_query_parameters(rmi_dev,
                    &data->sensors[0].sens_query,
                    query_base_addr + 1) >= 0) {

        pr_alert("%s, query1: 0x%X, query2: 0x%X, query3: 0x%X, query4: 0x%X\n", __func__,
                    data->sensors[0].sens_query.f11_2d_query1__4[0],
                    data->sensors[0].sens_query.f11_2d_query1__4[1],
                    data->sensors[0].sens_query.f11_2d_query1__4[2],
                    data->sensors[0].sens_query.f11_2d_query1__4[3]);
    }

    pdata->gpio_reset(pdata->gpio_data);

    if(!delayed_work_pending(&reset_wk)) {
        schedule_delayed_work(&reset_wk, msecs_to_jiffies(500));
    }

	pr_alert("%s, Resume exit with %d\n", __func__, retval);
	return retval;
}
#endif /* RESUME_REZERO */
					...
					char *buf)
{
	...
	fc = to_rmi_function_container(dev);
	instance_data =  fc->data;
	ctrl = &(instance_data->dev_controls);

	rmi_dev = fc->rmi_dev;

	control_base_addr = fc->fd.control_base_addr;
	f11_read_control_regs(rmi_dev, ctrl, control_base_addr);

    len += snprintf(buf + len, PAGE_SIZE, "ctrl0 : 0x%X\n", ctrl->ctrl0_9[0]);
    len += snprintf(buf + len, PAGE_SIZE, "ctrl1 : 0x%X\n", ctrl->ctrl0_9[1]);
    len += snprintf(buf + len, PAGE_SIZE, "ctrl2 : 0x%X\n", ctrl->ctrl0_9[2]);
    len += snprintf(buf + len, PAGE_SIZE, "ctrl3 : 0x%X\n", ctrl->ctrl0_9[3]);
    len += snprintf(buf + len, PAGE_SIZE, "ctrl4 : 0x%X\n", ctrl->ctrl0_9[4]);
    len += snprintf(buf + len, PAGE_SIZE, "ctrl5 : 0x%X\n", ctrl->ctrl0_9[5]);
    len += snprintf(buf + len, PAGE_SIZE, "ctrl6 : 0x%X\n", ctrl->ctrl0_9[6]);
    len += snprintf(buf + len, PAGE_SIZE, "ctrl7 : 0x%X\n", ctrl->ctrl0_9[7]);
    len += snprintf(buf + len, PAGE_SIZE, "ctrl8 : 0x%X\n", ctrl->ctrl0_9[8]);
    len += snprintf(buf + len, PAGE_SIZE, "ctrl9 : 0x%X\n", ctrl->ctrl0_9[9]);

    if (ctrl->ctrl10) {
        len += snprintf(buf + len, PAGE_SIZE, "ctrl10: 0x%X\n", ctrl->ctrl10->reg);
    }

    if (ctrl->ctrl11) {
        len += snprintf(buf + len, PAGE_SIZE, "ctrl11: 0x%X\n", ctrl->ctrl11->reg);
    }

    if (ctrl->ctrl12) {
        len += snprintf(buf + len, PAGE_SIZE, "ctrl12: 0x%X\n", ctrl->ctrl12->reg);
    }

    if (ctrl->ctrl14) {
        len += snprintf(buf + len, PAGE_SIZE, "ctrl14: 0x%X\n", ctrl->ctrl14->reg);
    }

    if (ctrl->ctrl15) {
        len += snprintf(buf + len, PAGE_SIZE, "ctrl15: 0x%X\n", ctrl->ctrl15->reg);
    }

    if (ctrl->ctrl16) {
        len += snprintf(buf + len, PAGE_SIZE, "ctrl16: 0x%X\n", ctrl->ctrl16->reg);
    }

    if (ctrl->ctrl17) {
        len += snprintf(buf + len, PAGE_SIZE, "ctrl17: 0x%X\n", ctrl->ctrl17->reg);
    }

    if (ctrl->ctrl18_19) {
        len += snprintf(buf + len, PAGE_SIZE, "ctrl18: 0x%X\n", ctrl->ctrl18_19->reg[0]);
        len += snprintf(buf + len, PAGE_SIZE, "ctrl19: 0x%X\n", ctrl->ctrl18_19->reg[1]);
    }

    if (ctrl->ctrl20_21) {
        len += snprintf(buf + len, PAGE_SIZE, "ctrl20: 0x%X\n", ctrl->ctrl20_21->reg[0]);
        len += snprintf(buf + len, PAGE_SIZE, "ctrl21: 0x%X\n", ctrl->ctrl20_21->reg[1]);
    }
					...
					char *buf)
{
    ...
    rmi_f11_get_query_parameters(rmi_dev,
                &instance_data->sensors[0].sens_query,
                query_base_addr + 1);

    len += snprintf(buf + len, PAGE_SIZE, "query1: 0x%X\n", 
                instance_data->sensors[0].sens_query.f11_2d_query1__4[0]);
    len += snprintf(buf + len, PAGE_SIZE, "query2: 0x%X\n", 
                instance_data->sensors[0].sens_query.f11_2d_query1__4[1]);
    len += snprintf(buf + len, PAGE_SIZE, "query3: 0x%X\n", 
                instance_data->sensors[0].sens_query.f11_2d_query1__4[2]);
    len += snprintf(buf + len, PAGE_SIZE, "query4: 0x%X\n", 
                instance_data->sensors[0].sens_query.f11_2d_query1__4[3]);

    if (instance_data->sensors[0].sens_query.has_abs) {
        len += snprintf(buf + len, PAGE_SIZE, "query5: 0x%X\n", 
                    instance_data->sensors[0].sens_query.f11_2d_query5);
    }

    if (instance_data->sensors[0].sens_query.has_rel) {
        len += snprintf(buf + len, PAGE_SIZE, "query6: 0x%X\n", 
                    instance_data->sensors[0].sens_query.f11_2d_query6);
    }

    if (instance_data->sensors[0].sens_query.has_gestures) {
        len += snprintf(buf + len, PAGE_SIZE, "query7: 0x%X\n", 
                    instance_data->sensors[0].sens_query.f11_2d_query7__8[0]);
        len += snprintf(buf + len, PAGE_SIZE, "query8: 0x%X\n", 
                    instance_data->sensors[0].sens_query.f11_2d_query7__8[1]);
    }


    if (instance_data->dev_query.has_query9) {
        len += snprintf(buf + len, PAGE_SIZE, "query9: 0x%X\n", 
                    instance_data->sensors[0].sens_query.query9);
    }

    if (instance_data->sensors[0].sens_query.has_touch_shapes) {
        len += snprintf(buf + len, PAGE_SIZE, "query10: 0x%X\n", 
                    instance_data->sensors[0].sens_query.f11_2d_query10);
    }
				     ...
				     const char *buf, size_t count)
{
    ...
    ctrl->ctrl0_9[0] = (u8)newValue;
    error = rmi_write_block(rmi_dev, control_base_addr,
                &(ctrl->ctrl0_9[0]),
                1);
    if (error < 0)
      return error;
				     ...
				     const char *buf, size_t count)
{
    ...
    ctrl->ctrl0_9[1] = (u8)newValue;
    error = rmi_write_block(rmi_dev, control_base_addr+ 1,
                &(ctrl->ctrl0_9[1]),
                1);
    if (error < 0)
      return error;
				     ...
				     const char *buf, size_t count)
{
    ...
    ctrl->ctrl0_9[2] = (u8)newValue;
    error = rmi_write_block(rmi_dev, control_base_addr + 2,
                &(ctrl->ctrl0_9[2]),
                1);
    if (error < 0)
      return error;
				     ...
				     const char *buf, size_t count)
{
    ...
    ctrl->ctrl0_9[3] = (u8)newValue;
    error = rmi_write_block(rmi_dev, control_base_addr + 3,
                &(ctrl->ctrl0_9[3]),
                1);
    if (error < 0)
      return error;
