...
static struct ddbg_register ddbg_pm2xxx_registers[] = {
	...
	DDBG_REG("PM2XXX_LED_CTRL_REG", PM2XXX_LED_CTRL_REG, DDBG_RW),
...
static int pm2xxx_ddbg_write(struct ddbg_target *ddbg, u32 val)
{
	int err;
	struct pm2xxx_charger *pm2xxx_ddbg;
	...
	pm2xxx_ddbg = (struct pm2xxx_charger *) ddbg->dev_data;
	if (!pm2xxx_ddbg) {
		err = -ENXIO;
		goto out;
	}

	err = pm2xxx_reg_write(pm2xxx_ddbg, ddbg->curr->offset, val);
...
static int pm2xxx_ddbg_read(struct ddbg_target *ddbg, u32 *val)
{
	int err;
	struct pm2xxx_charger *pm2xxx_ddbg;
	...
	err = pm2xxx_reg_read(pm2xxx_ddbg, ddbg->curr->offset, (u8 *)val);
