...
static int mmio_board_data_init(struct mmio_platform_data *pdata)
{
	...
	extra->gpio_uib1.name = "UIB_GPIO1";
	extra->gpio_uib1.gpio = MOP500_EGPIO(6);
...
/*
 * mmio_uib_init() - Initialize uib_gpio1 when using UIBT.
 *
 * This function is customizable.
 * It allows to reserve regulators / gpios that will be used.
 * It is called from mmio_platform_init function.
 */
static int mmio_uib_init(struct mmio_platform_data *pdata)
{
	int err = 0;
	struct mmio_board_data *extra = pdata->extra;
	...
	if (machine_is_u9540() && uib_is_u9540uibt_v1()) {
		/* Initialize UIB_GPIO1 */
		dev_dbg(pdata->dev,
			"Request %s gpio %d.\n",
			extra->gpio_uib1.name,
			extra->gpio_uib1.gpio);
		err = gpio_request(
			extra->gpio_uib1.gpio,
			extra->gpio_uib1.name);
		if (err) {
		...
		err = gpio_direction_output(extra->gpio_uib1.gpio, 0);
		if (err) {
			dev_err(pdata->dev,
				"Unable to set GPIO %d\n",
				extra->gpio_uib1.gpio);
...
/*
 * mmio_uib_exit() - De-initialize uib_gpio1 when using UIBT.
 *
 * This function is customizable.
 * It allows to free regulators / gpios reserved during init.
 * It is called from mmio_platform_exit function.
 */
static void mmio_uib_exit(struct mmio_platform_data *pdata)
{
	struct mmio_board_data *extra = pdata->extra;
	...
	if (machine_is_u9540() && uib_is_u9540uibt_v1())
		gpio_free(extra->gpio_uib1.gpio);
...
#define CAMERA_BACK_RIGHT  0x1
#define CAMERA_FRONT_RIGHT 0x2
#define CAMERA_BACK_LEFT   0x4
#define CAMERA_FRONT_LEFT  0x8
...
static int mmio_power_enable(struct mmio_platform_data *pdata)
{
	...
	if (machine_is_u9540()) {
		...
		if (pdata->camera_slot == PRIMARY_CAMERA) {
			...
			if (uib_is_u9540uibt_v1()) {
				/* UIB_GPIO1 disable */
				dev_dbg(pdata->dev, "UIB_GPIO1 disable\n");
				gpio_set_value_cansleep(
						extra->gpio_uib1.gpio, 0);
				camera_slot = CAMERA_BACK_LEFT;
			} else
			...
			if (uib_is_u9540uibt_v1()) {
				/* UIB_GPIO1 enable */
				dev_dbg(pdata->dev, "UIB_GPIO1 enable\n");
				gpio_set_value_cansleep(
						extra->gpio_uib1.gpio, 1);
				camera_slot = CAMERA_FRONT_RIGHT;
			} else
